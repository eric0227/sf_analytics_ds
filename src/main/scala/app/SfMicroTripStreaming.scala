...
object SfMicroTripStreaming extends App {
  ...
  val keyValueDF = spark.readStream
    ...
    //.option("checkpointLocation", "_checkpoint/sf-microtrip")
    .load()
...
// HBase  Sink -- microtrip
  val microtripWriteStream = spark.sql(
    ...
    //.option("checkpointLocation", "_checkpoint/microtrip")
  ...
  val tripWriteStream = spark.sql(
    ...
    .format("spark.sink.HBaseSinkProvider")
    //.option("checkpointLocation", "_checkpoint/trip")
  ...
  spark.sql(
    ...
    .format("spark.sink.HBaseSinkProvider")
    //.option("checkpointLocation", "_checkpoint/event")
